...
(() => {
 ...
 function files(){
  ...
  page('ファイル',`<div class="pd-folder-bar"><select id="pd-file-folder" aria-label="フォルダ"><option value="all">すべてのファイル</option><option value="" ${fileFolder===''?'selected':''}>未分類</option>${fileFolders().map(x=>`<option value="${esc(x)}" ${fileFolder===x?'selected':''}>${esc(x)}</option>`).join('')}</select>${button('pdFileFolders','フォルダを管理','files')}</div>${A.search('file-search','名前・内容を検索')}${tabs([['all','すべて'],['txt','TXT'],['md','MD'],['json','JSON'],['csv','CSV']],fileKind,'pdFileKind')}<div class="pd-file-toolbar"><span>${entries.length}件 · 本文 ${(entries.reduce((n,x)=>n+x.bytes,0)/1024).toFixed(1)}KB</span><select id="pd-file-sort" aria-label="並べ替え"><option value="date">更新順</option><option value="name">名前順</option><option value="size">サイズ順</option></select></div><div class="files-grid" id="files-grid"></div><button class="secondary-button pd-wide" data-action="fileImport">テキストを読み込む</button><p class="pd-caption">TXT・MD・JSON・CSV · 1件100KB・一度に20件まで</p>`,button('fileNew','ファイルを作成','plus'));
  const render=()=>{
