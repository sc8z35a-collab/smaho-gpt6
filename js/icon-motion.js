...
(() => {
  ...
  A.iconMotionPanel = () => `<section class="motion-panel" aria-label="アイコンの動き">
    <div class="motion-kicker"><span>VECTOR MOTION / 30 APPS</span><span class="motion-quality">FHD</span></div>
    <h3>FHD MOVE APP</h3><p class="motion-lead">いつものアイコンに、<br>小さな命を。</p>
    <div class="motion-samples" aria-hidden="true">${['weather','camera','settings','messages'].map(tile).join('')}</div>
    <p class="motion-copy">レンズの絞り、流れる雲、精密な歯車。<br>描き込まれた質感は、そのままに。</p>
    <div class="motion-mode-picker" role="group" aria-label="アイコンの動作モード">
      <button data-action="chooseIconMotion" data-value="static" aria-pressed="${!enabled()}">静止<span>ORIGINAL</span></button>
      <button data-action="chooseIconMotion" data-value="fhd" aria-pressed="${enabled()}">動かす<span>FHD MOVE APP</span></button>
    </div><p class="motion-status" role="status">${status()}</p>
    <button class="motion-gallery-link" data-action="iconMotionGallery">30個の動きを、大きく見る <span aria-hidden="true">↗</span></button>
    <p class="motion-footnote">拡大しても鮮明なSVG描画。FHDはモード名です。<br>装飾アニメーションであり、通信・録音・健康状態の表示ではありません。</p>
  </section>`;
  A.actions.iconMotion = () => A.overlay(`${A.overlayTitle('アイコンの動き')}${A.iconMotionPanel()}<button class="reset-layout" data-action="appearance">光と奥行き・動きを抑える設定</button>`, 'motion-overlay');
  let selected = 'camera';
  const renderDetail = () => {
  ...
  A.actions.iconMotionGallery = () => {
    A.overlay(`${A.overlayTitle('Motion atelier')}<p class="motion-gallery-intro">FHD MOVE APP <span>30の小さな世界</span></p>
      ...
      <p class="motion-footnote">アイコンを選ぶと拡大表示。動きは装飾です。<br>画面外では自動停止します。</p><button class="reset-layout" data-action="iconMotion">モード設定に戻る</button>`, 'motion-overlay motion-gallery-overlay');
    renderDetail();
  };
