...
(() => {
  ...
  const skCustomPalettes=()=>read('sketchPalettes').filter(p=>p&&typeof p.id==='string'&&typeof p.name==='string'&&Array.isArray(p.colors)).slice(0,8).map(p=>({id:p.id,name:p.name.slice(0,30),colors:[...new Set(p.colors.filter(c=>typeof c==='string'&&/^#[0-9a-f]{6}$/i.test(c)).map(c=>c.toLowerCase()))].slice(0,12)}));
  ...
  function sketchEditor(){
    ...
    page('sketch',`<div class="sk-title-row">${skButton('evSketchHome','‹','', 'aria-label="作品一覧に戻る" title="作品一覧"')}<input class="ev-sketch-title" id="ev-sketch-title" aria-label="スケッチ名" maxlength="80" value="${esc(drawing.title)}">${skButton('evSketchFavorite',skIcon('star'),'',`aria-label="お気に入り" aria-pressed="${drawing.favorite}"`)}</div>
      ...
      <section class="sk-inspector" id="sk-panel-color" aria-labelledby="sk-tab-color" ${skPanel==='color'?'':'hidden'}><div class="sk-panel-heading"><h2>カラーパレット</h2>${skButton('evSketchPanelClose','閉じる')}</div><label class="sk-inline-label">配色 <select id="sk-palette-select"><option value="atelier">アトリエ</option><option value="earth">アース</option><option value="ocean">オーシャン</option></select></label><div class="ev-drawing-tools sk-palette" id="sk-color-swatches"></div><div class="sk-color-custom"><input type="color" id="ev-pen-custom" aria-label="色を選択" value="${penColor}"><strong id="sk-color-code">${penColor.toUpperCase()}</strong><span>カスタムカラー</span></div><h3>最近使った色</h3><div id="sk-recent-colors" class="sk-palette ev-drawing-tools"></div><div class="sk-layer-actions">${skButton('evSketchPaletteSave','最近の色を配色に保存')}${skButton('evSketchPaletteEdit','配色を編集')}${skButton('evSketchPaletteDelete','配色を削除')}</div><p class="sk-hint">カスタム配色は8組・各12色まで。</p></section>
  ...
  A.actions.evSketchTool=el=>{if(!skTools.some(t=>t[0]===el.dataset.id))return;skFinish?.();skTool=el.dataset.id;if(['pen','pencil','marker','brush','airbrush'].includes(skTool))skLastBrush=skTool;skSelected=-1;skSelection.clear();skMarquee=null;$('#ev-canvas').dataset.tool=skTool;A.$$('[data-action=evSketchTool]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.id===skTool)));skToolStatus();skSelectionBar();paint();};
  ...
  A.actions.evSketchClipboardPaste=()=>{skFinish?.();if(!skClipboard.length||!skEditable()||!skCapacity(skClipboard.length,skClipboard.reduce((n,s)=>n+s.points.length,0)))return;const copies=skClone(skClipboard).map(s=>({...s,layer:drawing.activeLayer,dx:Math.min(1600,(s.dx||0)+20),dy:Math.min(1600,(s.dy||0)+20)}));skChange(()=>drawing.strokes.push(...copies));skTool='select';$('#ev-canvas').dataset.tool='select';skToolStatus();skRestoreSelection(copies);};
  ...
  A.actions.evSketchSelectionExport=()=>{
    ...
    const doc={...drawing,strokes:indices.map(i=>drawing.strokes[i])},source=document.createElement('canvas');source.width=source.height=1600;skRender(source,doc,{transparent:true});const crop=document.createElement('canvas');crop.width=(right-left)*2;crop.height=(bottom-top)*2;crop.getContext('2d').drawImage(source,left*2,top*2,crop.width,crop.height,0,0,crop.width,crop.height);crop.toBlob(blob=>{if(blob)A.download(blob,skFilename()+'-selection.png');else A.toast('書き出せませんでした');},'image/png');
  };
