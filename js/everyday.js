...
(() => {
  ...
  let skPanel='',skAxis='vertical',skDash=false,skReference=null,skTrace=false,skReferenceOpacity=.35;
  ...
  const skPresets=()=>read('sketchBrushPresets').filter(p=>p&&typeof p.name==='string'&&['pen','pencil','marker','brush','airbrush','erase'].includes(p.tool)).slice(0,6);
  ...
  function skGallery(){
    skFinish?.();drawing=null;skSelected=-1;skReference=null;skTrace=false;
  ...
  function skSelectionBar(){const el=$('#sk-selection');if(!el)return;const s=drawing.strokes[skSelected];el.hidden=!s;el.innerHTML=s?`<span>${esc(skTools.find(t=>t[0]===s.tool)?.[1]||'描画')}を選択</span>${skButton('evSketchObjectStyle','変形・色')}${skButton('evSketchArrange','整列・移動')}${skButton('evSketchObjectOrder','手前へ','front')}${skButton('evSketchObjectOrder','奥へ','back')}${skButton('evSketchObjectCopy','複製')}${skButton('evSketchObjectDelete','削除')}${s.tool==='text'?skButton('evSketchTextEdit','文字編集'):''}`:'選択 → ドラッグで移動';}
  ...
  function sketchEditor(){
    ...
    page('sketch',`<div class="sk-title-row">${skButton('evSketchHome','‹','', 'aria-label="作品一覧に戻る" title="作品一覧"')}<input class="ev-sketch-title" id="ev-sketch-title" aria-label="スケッチ名" maxlength="80" value="${esc(drawing.title)}">${skButton('evSketchFavorite',skIcon('star'),'',`aria-label="お気に入り" aria-pressed="${drawing.favorite}"`)}</div>
      ...
      <div class="sk-panel-tabs" role="toolbar" aria-label="制作パネル">${[['brush','ブラシ'],['color','色'],['layers','レイヤー'],['paper','用紙'],['reference','参照']].map(([id,label])=>skButton('evSketchPanel',skIcon(id)+`<span>${label}</span>`,id,`id="sk-tab-${id}" aria-controls="sk-panel-${id}" aria-expanded="${skPanel===id}"`)).join('')}</div>
      ...
      <section class="sk-inspector" id="sk-panel-color" aria-labelledby="sk-tab-color" ${skPanel==='color'?'':'hidden'}><div class="sk-panel-heading"><h2>カラーパレット</h2>${skButton('evSketchPanelClose','閉じる')}</div><label class="sk-inline-label">配色 <select id="sk-palette-select"><option value="atelier">アトリエ</option><option value="earth">アース</option><option value="ocean">オーシャン</option></select></label><div class="ev-drawing-tools sk-palette" id="sk-color-swatches"></div><div class="sk-color-custom"><input type="color" id="ev-pen-custom" aria-label="色を選択" value="${penColor}"><strong id="sk-color-code">${penColor.toUpperCase()}</strong><span>カスタムカラー</span></div><h3>最近使った色</h3><div id="sk-recent-colors" class="sk-palette ev-drawing-tools"></div></section>
      <section class="sk-inspector" id="sk-panel-layers" aria-labelledby="sk-tab-layers" ${skPanel==='layers'?'':'hidden'}><div class="sk-panel-heading"><h2>レイヤー <span id="sk-layer-count">${drawing.layers.length} / 8</span></h2>${skButton('evSketchPanelClose','閉じる')}</div><div id="sk-layers"></div><div class="sk-layer-actions">${skButton('evSketchLayerAdd','追加')}${skButton('evSketchLayerCopy','複製')}${skButton('evSketchLayerUp','手前へ')}${skButton('evSketchLayerDown','奥へ')}${skButton('evSketchLayerClear','内容消去')}${skButton('evSketchLayerDelete','削除')}</div><p class="ev-caption">上が手前。消しゴムは選択レイヤーだけに適用。</p></section>
      <section class="sk-inspector" id="sk-panel-paper" aria-labelledby="sk-tab-paper" ${skPanel==='paper'?'':'hidden'}><div class="sk-panel-heading"><h2>用紙と表示</h2>${skButton('evSketchPanelClose','閉じる')}</div><div class="sk-paper-choices">${[['plain','無地'],['grid','方眼'],['dots','ドット'],['ruled','罫線']].map(([id,label])=>skButton('evSketchPaperQuick',`<i class="sk-paper-sample ${id}"></i><span>${label}</span>`,id,`aria-pressed="${drawing.paper===id}"`)).join('')}</div><div class="sk-layer-actions">${skButton('evSketchPaper','用紙の色・模様')}${skButton('evSketchFit','表示をリセット')}${skButton('evSketchHelp','使い方')}</div><p class="ev-caption">800 × 800 px · 最大 2400 px で書き出し</p></section>
      ...
      <div class="sk-selection" id="sk-selection"></div>`,btn('evSketchMenu','スケッチのメニュー','share'));
    $('#ev-sketch-title').oninput=e=>{drawing.title=e.target.value;sketchSave();};
    $('#ev-pen-width').oninput=e=>{penWidth=Number(e.target.value);$('#sk-width-value').textContent=penWidth;skBrushPreview();};
    ...
    $('#sk-dash').onchange=e=>skDash=e.target.checked;$('#sk-axis').onchange=e=>skAxis=e.target.value;
    $('#sk-palette-select').onchange=skPaletteRender;
  ...
  A.actions.evSketchNew=()=>{A.cleanup();skReference=null;skTrace=false;skPanel='';skFocus=false;drawing=skNormalize({id:A.id(),title:'スケッチ',strokes:[],updated:Date.now()});skUndo=[];skRedo=[];skZoom=1;skSelected=-1;sketchSave();sketchEditor();};
  A.actions.evSketchOpen=el=>{A.cleanup();skReference=null;skTrace=false;skPanel='';skFocus=false;try{const raw=skList().find(d=>d.id===el.dataset.id);if(!raw)return;drawing=skNormalize(raw);skUndo=[];skRedo=[];skZoom=1;skSelected=-1;sketchEditor();}catch(e){A.toast(e.message);}};
  ...
  A.actions.evSketchTool=el=>{if(!skTools.some(t=>t[0]===el.dataset.id))return;skFinish?.();skTool=el.dataset.id;skSelected=-1;$('#ev-canvas').dataset.tool=skTool;A.$$('[data-action=evSketchTool]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.id===skTool)));skToolStatus();skSelectionBar();paint();};
  function skToolStatus(){
    const label=skTools.find(t=>t[0]===skTool)?.[1]||'ペン';const el=$('#sk-active-tool');if(el)el.textContent=label;
  ...
  function skPanelShow(id){skFinish?.();skPanel=id;for(const name of ['brush','color','layers','paper','reference']){const el=$('#sk-panel-'+name);if(el)el.hidden=id!==name;$('#sk-tab-'+name)?.setAttribute('aria-expanded',String(id===name));}if(id&&skFocus)A.actions.evSketchFocus();}
  ...
  A.actions.evSketchQuick=el=>{A.actions.evSketchTool(el);if(el.dataset.id==='rect')skPanelShow('brush');};
  A.actions.evSketchFit=()=>{skFinish?.();skZoom=1;$('#sk-zoom').value='1';$('#sk-canvas-size').style.width='100%';$('#sk-viewport').scrollTo(0,0);};
  function skPaletteRender(){const render=colors=>colors.map(c=>skButton('evPenColor','',c,`class="ev-swatch" style="--swatch:${c}" aria-label="色 ${c}" aria-pressed="${penColor===c}"`)).join('');const el=$('#sk-color-swatches');if(el)el.innerHTML=render(skPalettes[$('#sk-palette-select').value]||skColors);const recent=$('#sk-recent-colors');if(recent)recent.innerHTML=render(skRecent())||'<span class="sk-hint">描くと、ここに色が残ります。</span>';skSetColor(penColor);}
  ...
  A.actions.evSketchPresetApply=el=>{const p=skPresets()[Number(el.dataset.id)];if(!p)return;skFinish?.();penWidth=skNumber(p.width,1,120,4);skOpacity=skNumber(p.opacity,.01,1,1);skPressure=!!p.pressure;skSmooth=!!p.smooth;skTool=p.tool;penColor=skColor(p.color);skPanel='brush';sketchEditor();};
  ...
  A.actions.evSketchTemplate=()=>A.form('新しい用紙',field('作品名','title','スケッチ','text','required maxlength="80"')+select('テンプレート','template',[['plain','白紙 / アイデア'],['grid','方眼 / 設計'],['dots','ドット / レタリング'],['ruled','罫線 / 手書きノート']],'plain')+select('用紙色','tone',[['cream','クリーム'],['white','白'],['dark','ダーク']],'cream'),v=>{if(!v.title.trim())return false;A.cleanup();skReference=null;skTrace=false;skPanel='';skFocus=false;drawing=skNormalize({id:A.id(),title:v.title,paper:v.template,tone:v.tone,strokes:[]});skUndo=[];skRedo=[];skZoom=1;skSelected=-1;sketchSave();sketchEditor();});
  A.actions.evSketchArrange=()=>{const index=skSelected,s=drawing?.strokes[index];if(!s||!skEditable())return;const layers=drawing.layers.filter(l=>!l.locked&&l.visible);A.form('整列・レイヤー移動',select('キャンバスに整列','align',[['none','位置を維持'],['center','中央'],['horizontal','左右中央'],['vertical','上下中央'],['left','左端'],['right','右端'],['top','上端'],['bottom','下端']],'none')+select('移動先レイヤー','layer',layers.map(l=>[l.id,l.name]),s.layer),v=>{const target=drawing.layers.find(l=>l.id===v.layer);if(!skEditable()||!target||target.locked||!target.visible)return false;const b=skBounds(s);let dx=0,dy=0;if(['center','horizontal'].includes(v.align))dx=400-b.x-b.w/2;if(['center','vertical'].includes(v.align))dy=400-b.y-b.h/2;if(v.align==='left')dx=-b.x;if(v.align==='right')dx=800-b.x-b.w;if(v.align==='top')dy=-b.y;if(v.align==='bottom')dy=800-b.y-b.h;skChange(()=>{s.dx=skNumber((s.dx||0)+dx,-1600,1600,0);s.dy=skNumber((s.dy||0)+dy,-1600,1600,0);s.layer=target.id;drawing.activeLayer=target.id;skLayerPanel();});skSelected=index;skSelectionBar();paint();});};
  ...
  A.actions.evSketchHelp=()=>A.overlay(`${A.overlayTitle('スケッチの使い方')}<div class="ev-menu-sheet sk-help"><h3>新しい制作パネル</h3><p>キャンバス下のブラシ・色・レイヤー・用紙・参照で必要な設定だけを開きます。筆は速さや筆圧で線幅が変化し、エアブラシは柔らかく重なります。ブラシ設定は6件、最近使った色は8色までブラウザ内に保存します。</p><h3>描画アシスト</h3><p>対称描画は左右・上下・四方向。Shift＋ドラッグで正方形・正円・45度刻みの線を描けます。図形の破線、文字の3書体、レイヤーの乗算・スクリーン・オーバーレイにも対応。選択した要素は「整列・移動」で位置やレイヤーを変更できます。</p><h3>参照画像</h3><p>PNG・JPEG・WebP（8MB、1600万画素まで）を見本や下絵に表示します。編集画面だけの表示で、保存・画像書き出し・JSONには含めません。作品一覧へ戻ると解除します。</p><h3>描画と編集</h3><p>図形はドラッグ、文字は配置場所をタップ。選択ツールは現在のレイヤーの要素を囲み枠で選択し、移動・複製・削除できます。「変形・色」で回転・拡大縮小・反転・色の変更、「手前へ／奥へ」でレイヤー内の重なりを調整できます。選択判定は外接矩形です。</p><h3>レイヤーと拡大</h3><p>最大8レイヤー。自由／固定でロックを切り替えます。拡大時は「手のひら」ツールでドラッグして移動。「集中表示」で設定を隠し、同じボタンで戻せます。消しゴムは現在のレイヤーだけを透明にします。旧作品の消しゴムは見た目を保つため用紙色のままです。</p><h3>ショートカット</h3><p>Ctrl / ⌘ + Z：元に戻す<br>Ctrl / ⌘ + Shift + Z、Ctrl + Y：やり直す<br>B：ペン、E：消しゴム、V：選択、I：スポイト、P：手のひら<br>Delete：選択要素を削除<br>[ / ]：太さを変更、矢印：選択要素を1px移動（Shiftで10px）<br>Esc：選択解除・パネルを閉じる、0：表示リセット</p><h3>保存の範囲</h3><p>自動保存はこのブラウザのみ。履歴は編集中のみ最大30操作（容量により減少）。1作品1,000要素・合計100,000点、1ストローク5,000点。容量不足時の未保存作品はタブ内で保持しますが、再読み込み前にJSONで退避してください。</p></div>`,'sheet-overlay');
  document.addEventListener('keydown',e=>{
    ...
    if(mod&&['z','y'].includes(key)){e.preventDefault();(key==='y'||e.shiftKey?A.actions.evSketchRedo:A.actions.evSketchUndo)();}
      ...
      if(key==='escape'){e.preventDefault();skFinish?.();skSelected=-1;skPanelShow('');skSelectionBar();paint();return;}
      if(key==='0'){e.preventDefault();A.actions.evSketchFit();return;}
      if(['[',']'].includes(key)){e.preventDefault();penWidth=Math.max(1,Math.min(120,penWidth+(key==='['?-1:1)));$('#ev-pen-width').value=penWidth;$('#sk-width-value').textContent=penWidth;skBrushPreview();return;}
      if(['arrowleft','arrowright','arrowup','arrowdown'].includes(key)&&skSelected>=0){e.preventDefault();if(!skEditable())return;const index=skSelected,s=drawing.strokes[index],n=e.shiftKey?10:1;skChange(()=>{s.dx=skNumber((s.dx||0)+(key==='arrowleft'?-n:key==='arrowright'?n:0),-1600,1600,0);s.dy=skNumber((s.dy||0)+(key==='arrowup'?-n:key==='arrowdown'?n:0),-1600,1600,0);});skSelected=index;skSelectionBar();paint();return;}
