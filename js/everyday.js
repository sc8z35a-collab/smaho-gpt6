...
(() => {
  ...
  function expensesApp(){const rows=read('expenses').filter(x=>x.date.startsWith(expenseMonth)).sort((a,b)=>b.date.localeCompare(a.date)), totals=expenseTotals(rows),budget=A.load('expenseBudget',0);
    page('expenses',`<label class="ev-month"><input aria-label="表示月" type="month" id="ev-expense-month" value="${expenseMonth}"></label><div class="ev-hero teal"><span>今月の支出</span><strong>${money(totals.expense)}</strong><div class="ev-hero-foot"><span>収入 ${money(totals.income)}</span><span>収支 ${money(totals.income-totals.expense)}</span></div></div><button class="ev-budget" data-action="evExpenseBudget"><span>月の予算</span><strong>${!budget?'設定':totals.expense>budget?money(totals.expense-budget)+' 超過':money(budget-totals.expense)+' 残り'}</strong></button>${budget?progress(totals.expense/budget*100,'予算の使用率'):''}${section('内訳')}<div class="ev-card ev-breakdown">${categories.map(([id,name])=>{const n=rows.filter(x=>x.kind==='expense'&&x.category===id).reduce((s,x)=>s+x.amount,0);return n?`<div><span>${name}</span><i><b style="width:${n/(totals.expense||1)*100}%"></b></i><strong>${money(n)}</strong></div>`:'';}).join('')||empty('支出なし')}</div>${section('履歴',btn('evExpenseExport','CSVを書き出す','download'))}<div class="ev-card">${rows.map(x=>`<button class="ev-row ev-wide" data-action="evExpenseEdit" data-id="${x.id}"><span class="ev-grow"><strong>${esc(x.note||categories.find(c=>c[0]===x.category)?.[1]||'収入')}</strong><small>${esc(x.date)} · ${x.kind==='income'?'収入':categories.find(c=>c[0]===x.category)?.[1]||'その他'}</small></span><strong class="${x.kind==='income'?'ev-positive':''}">${x.kind==='income'?'+':'−'}${money(x.amount)}</strong></button>`).join('')||empty('＋で収支を記録')}</div>`,btn('evExpenseEdit','収支を追加','plus'));
  ...
  expensesApp = function(){const rows=read('expenses').filter(x=>x.date.startsWith(expenseMonth)).sort((a,b)=>b.date.localeCompare(a.date)), totals=expenseTotals(rows),budget=getBudget();const previous=new Date(expenseMonth+'-15T12:00:00');previous.setMonth(previous.getMonth()-1);const last=expenseTotals(read('expenses').filter(x=>x.date.startsWith(dateKey(previous).slice(0,7)))).expense;
    page('expenses',`<label class="ev-month"><input aria-label="表示月" type="month" id="ev-expense-month" value="${expenseMonth}"></label><div class="ev-hero teal"><span>今月の支出</span><strong>${money(totals.expense)}</strong><div class="ev-hero-foot"><span>収入 ${money(totals.income)}</span><span>収支 ${money(totals.income-totals.expense)}</span></div><div class="ev-comparison">前月比 ${totals.expense-last>0?'+':''}${money(totals.expense-last)}</div></div><button class="ev-budget" data-action="evExpenseBudget"><span>月の予算</span><strong class="${budget.total&&totals.expense>budget.total?'ev-over':''}">${!budget.total?'設定':totals.expense>budget.total?money(totals.expense-budget.total)+' 超過':money(budget.total-totals.expense)+' 残り'}</strong></button>${budget.total?progress(totals.expense/budget.total*100,'予算の使用率'):''}${section('内訳')}<div class="ev-card ev-breakdown">${categories.map(([id,name])=>{const n=rows.filter(x=>x.kind==='expense'&&x.category===id).reduce((s,x)=>s+x.amount,0),cap=budget.categories?.[id]||0;return n||cap?`<div class="ev-category-budget"><span>${name}</span><i><b class="${cap&&n>cap?'over':''}" style="width:${Math.min(100,n/(cap||totals.expense||1)*100)}%"></b></i><strong>${money(n)}${cap?`<small>/ ${money(cap)}</small>`:''}</strong></div>`:'';}).join('')||empty('支出なし')}</div><button class="ev-recurring-link" data-action="evRecurringHome">${A.icon('refresh')}固定費<span>›</span></button>${section('履歴',btn('evExpenseExport','CSVを書き出す','download'))}${chips([['all','すべて'],['expense','支出'],['income','収入']],expenseKind,'evExpenseKind')}${A.search('ev-expense-search','内容を検索')}<div class="ev-card" id="ev-expense-rows"></div>`,btn('evExpenseEdit','収支を追加','plus'));
  ...
  A.searchAdditional = query => {
    const text=(...values)=>values.filter(v=>v!==undefined&&v!==null&&v!=='').map(String).join(' ');
    const sources=[['contacts','連絡先',x=>text(x.name),x=>text(x.phone,x.email),'evContactOpen'],['journal','日記',x=>text(x.title||x.date),x=>text(x.body),'evJournalEdit'],['reading','読書',x=>text(x.title),x=>text(x.author,x.note),'evBookOpen'],['shopping','買い物',x=>text(x.name),x=>x.quantity?x.quantity+'点':'','evShoppingEdit'],['habits','習慣',x=>text(x.name),()=>'', 'evHabitEdit']];
    return sources.map(([key,label,title,detail,action])=>{const items=read(key).filter(x=>x&&x.id).filter(x=>(title(x)+' '+detail(x)).normalize('NFKC').toLowerCase().includes(query.normalize('NFKC'))).slice(0,4);return items.length?`<p class="spotlight-label">${label}</p><div class="search-content-group">${items.map(x=>`<button data-action="evSearchOpen" data-app-id="${key}" data-target="${action}" data-id="${esc(x.id)}">${A.icon(key)}<span><strong>${esc(title(x))}</strong><small>${esc(detail(x).slice(0,65))}</small></span>${A.icon('arrow')}</button>`).join('')}</div>`:'';}).join('');
  };
