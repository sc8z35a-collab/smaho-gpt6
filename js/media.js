...
function showPhotoEditor(){
  ...
  img.onload=()=>{clearTimeout(timer);if(cancelled||photoEditor!==ed||!root.isConnected)return;ed.image=img;try{updatePhotoEditor();$('#pa-edit-save').disabled=false;$('#pa-edit-export').disabled=false;$('#pa-edit-status').textContent='';}catch{ed.image=null;$('#pa-edit-status').textContent='この写真は編集できません。端末に保存してから追加してください。';}};
