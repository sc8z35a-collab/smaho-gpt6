...
const opLabel={'+':'+','-':'−','*':'×','/':'÷'};
function calcApp(){A.statusTheme(true);$('#app-screen').classList.add('calc-app');A.view(A.nav('計算機',`<button data-action="calcHistory" style="color:#d3ba95" aria-label="計算履歴">${icon('clock')}</button>`)+`<div class="calc-display"><div class="calc-expression" id="calc-expression">${esc(calc.expression)}</div><div class="calc-result" id="calc-result">${esc(calc.entry)}</div></div><div class="calc-keypad">${[['AC','clear','utility'],['+/−','sign','utility'],['%','percent','utility'],['÷','/','operator'],['7','7',''],['8','8',''],['9','9',''],['×','*','operator'],['4','4',''],['5','5',''],['6','6',''],['−','-','operator'],['1','1',''],['2','2',''],['3','3',''],['+','+','operator'],['0','0','zero'],['.','.',''],['=','=','operator']].map(([label,v,c])=>`<button class="calc-key ${c}" data-action="calcKey" data-value="${v}">${label}</button>`).join('')}</div>`);const keydown=e=>{if(A.current!=='calculator'||!$('#overlay').hidden||!$('#calc-result')||e.ctrlKey||e.metaKey||e.altKey||e.isComposing||(e.target instanceof Element&&e.target.closest('input,textarea,select,[contenteditable="true"]')))return;let v=e.key;if(v==='Enter')v='=';if(v==='Backspace'){e.preventDefault();if(calc.entry==='エラー'){pressCalc('clear');return;}const entry=calc.entry.slice(0,-1);calc.entry=entry&&Number.isFinite(Number(entry))?entry:'0';calc.fresh=false;calc.lastOp=null;calc.lastOperand=null;if(!calc.operator)calc.expression='';updateCalc();return;}if(v==='Delete')v='clear';if('0123456789.+-*/='.includes(v)||v==='clear'){e.preventDefault();pressCalc(v);}};document.addEventListener('keydown',keydown);A.cleanups.push(()=>document.removeEventListener('keydown',keydown));}
...
else if(/^\d$/.test(v)){if(calc.fresh||calc.entry==='エラー'){calc.entry=v;calc.fresh=false;}else if(calc.entry.replace(/^-/,'').length<13)calc.entry=calc.entry==='0'?v:calc.entry==='-0'?'-'+v:calc.entry+v;}
else if(v==='.'){if(calc.fresh||calc.entry==='エラー'){calc.entry='0.';calc.fresh=false;}else if(!calc.entry.includes('.'))calc.entry+='.';}
else if(v==='sign'){if(calc.entry==='エラー')return;if(calc.fresh&&calc.operator){calc.entry='-0';calc.fresh=false;}else if(calc.entry!=='0')calc.entry=calc.entry.startsWith('-')?calc.entry.slice(1):'-'+calc.entry;}
// Standard calculator percent: after + / − it is a share of the first operand (50 + 10% = 55).
else if(v==='percent'){if(calc.entry==='エラー')return;const value=Number(calc.entry);calc.entry=formatNumber(calc.operator&&!calc.fresh&&['+','-'].includes(calc.operator)?calc.previous*value/100:value/100);}
...
else if(clockTab==='alarm')el.innerHTML=`<h1 class="app-title">アラーム</h1><p class="app-subtitle">ページ稼働中のみ。背面・ロック中は保証なし</p>${alarms.length?'':A.empty('＋でアラームを追加','alarm')}${alarms.map(a=>`<div class="alarm-row"><div><strong class="alarm-time" style="color:${a.enabled?'inherit':'#a9aab3'}">${esc(a.time)}</strong><small>${esc(a.label)} · 毎日</small></div><button class="toggle ${a.enabled?'on':''}" data-action="alarmToggle" data-id="${esc(a.id)}" aria-label="${esc(a.time)}のアラーム" aria-pressed="${a.enabled}"></button><button class="delete-small" data-action="alarmDelete" data-id="${esc(a.id)}" aria-label="${esc(a.time)}のアラームを削除">×</button></div>`).join('')}`;
...
A.apps.clock.render=arg=>{
  ...
  const keydown=e=>{if(e.code==='Space'&&!e.repeat&&A.current==='clock'&&clockTab==='timer'&&$('#overlay').hidden&&!(e.target instanceof Element&&e.target.closest('button,input,textarea,select,summary,[contenteditable]'))){e.preventDefault();A.actions.timerToggle();}};
...
// Alarm writes are copy-on-write: a failed save never changes the visible schedule.
...
if(!wallet||!Number.isFinite(wallet.balance)||!Array.isArray(wallet.transactions))wallet={balance:3240,transactions:[]};
wallet.transactions=wallet.transactions.filter(t=>t&&Number.isFinite(t.amount)&&Number.isFinite(t.date));
const commitWallet=next=>{if(!A.save('wallet',next))return false;wallet=next;return true;};
