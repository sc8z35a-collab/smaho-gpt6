...
(() => {
  ...
  const mailKey = 'mailStudio', mailPending = new Map();
  let mailFolder = 'drafts', mailQuery = '', mailSort = 'new', mailEditing = null, mailTimer;
  const mailText = (v, max) => typeof v === 'string' ? v.slice(0, max) : '';
  const mailDraft = v => ({id:mailText(v.id,100) || A.id(), to:mailText(v.to,1000), cc:mailText(v.cc,1000), bcc:mailText(v.bcc,1000), subject:mailText(v.subject,200), body:mailText(v.body,12000), starred:!!v.starred, trash:!!v.trash, updated:Number.isFinite(v.updated) ? v.updated : Date.now()});
  function mailStore() {
    const stored = A.load(mailKey, null);
    if (stored && stored.version === 1 && Array.isArray(stored.drafts)) return {...stored, signature:mailText(stored.signature,1000), drafts:stored.drafts.filter(v=>v && typeof v === 'object').map(mailDraft)};
    const old = A.load('externalMailDraft', {}) || {};
    return {version:1, signature:'', drafts:[old.to,old.subject,old.body].some(v=>typeof v === 'string' && v) ? [mailDraft({...old,id:'legacy-mail'})] : []};
  }
  ...
  function mailPersist(d) {
    const store = mailStore(), exists = store.drafts.some(x=>x.id===d.id);
    ...
    if (!A.save(mailKey,next)) return false;
    mailPending.delete(d.id); return true;
  }
  ...
  function mailEditor(d) {
    mailFlush(); mailEditing={...d}; A.statusTheme(false);
  ...
  A.apps.mail.render=()=>{
    mailLibrary();
    const flush=()=>mailFlush(), warn=e=>{if(mailPending.size){e.preventDefault();e.returnValue='';}};
    document.addEventListener('visibilitychange',flush); window.addEventListener('pagehide',flush); window.addEventListener('beforeunload',warn);
    A.cleanups.push(()=>{mailFlush();document.removeEventListener('visibilitychange',flush);window.removeEventListener('pagehide',flush);window.removeEventListener('beforeunload',warn);});
  };
  ...
  A.actions.msDelete=el=>A.confirm('下書きを完全削除','この操作は取り消せません。',()=>{const store=mailStore();if(A.save(mailKey,{...store,drafts:store.drafts.filter(d=>d.id!==el.dataset.id)})){mailPending.delete(el.dataset.id);mailLibrary();}});
  ...
  A.actions.msTools=()=>{
    ...
    $('#ms-signature-save').onclick=()=>{if(A.save(mailKey,{...mailStore(),signature:$('#ms-signature').value}))$('#ms-signature-status').textContent='署名を保存しました';else $('#ms-signature-status').textContent='保存失敗。内容をコピーして退避してください';};
