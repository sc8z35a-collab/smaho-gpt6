...
(() => {
  ...
  // Private address book: never reuse fictional demo contacts for native handoffs.
  let addressBook=A.load('addressBook',[]),contactFilter='all',selectedContact=null;
  function saveContacts(next){if(!A.save('addressBook',next))return false;addressBook=next;return true;}
  const contactRow=(c,action='contactDetail')=>`<button class="list-row contact-row" data-action="${action}" data-id="${esc(c.id)}"><span class="avatar">${esc(Array.from(c.name)[0]||'?')}</span><span class="row-main"><strong>${esc(c.name)}${c.favorite?' ★':''}</strong><small>${esc(c.company||c.number||c.email||'連絡先')}</small></span><span class="chevron">›</span></button>`;
  A.actions.nativePhoneHome=()=>A.open('phone');
  A.actions.contactsHome=()=>{
    A.statusTheme(false);
    A.view(A.nav('連絡先',`<button data-action="contactNew" aria-label="連絡先を追加">${A.icon('plus')}</button>`,'nativePhoneHome','電話')+`<div class="app-content work-app contacts-work"><span class="work-eyebrow">YOUR PEOPLE, CLOSE BY</span><h1 class="app-title">つながりを、ここに。</h1><p class="app-subtitle">${addressBook.length}件 · このブラウザにのみ保存</p>${UI.chips([['all','すべて'],['favorites','よく使う項目']],contactFilter,'contactFilter')}${A.search('address-search','名前・電話番号・メールを検索')}<div id="address-list"></div><p class="work-note">自分で登録した連絡先のみ表示します。<br>端末の連絡先の読み取り・クラウド同期は行いません。</p></div>`);
    const render=q=>{$('#address-list').innerHTML=addressBook.filter(c=>(contactFilter==='all'||c.favorite)&&(c.name+' '+c.number+' '+c.email+' '+c.company).toLowerCase().includes(q.toLowerCase())).sort((a,b)=>a.name.localeCompare(b.name,'ja')).map(c=>contactRow(c)).join('')||A.empty('連絡先がありません。<br>右上の＋から登録できます。','user');};
    render('');$('#address-search').oninput=e=>render(e.target.value);
  };
  A.actions.contactFilter=el=>{contactFilter=el.dataset.value;A.actions.contactsHome();};
  function contactEditor(id){const c=addressBook.find(c=>c.id===id);A.form(c?'連絡先を編集':'新しい連絡先',UI.field('名前','name',c?.name||'','text','required maxlength="60"')+UI.field('電話番号（任意）','number',c?.number||'','tel','maxlength="25"')+UI.field('メール（任意）','email',c?.email||'','email','maxlength="254"')+UI.field('会社・グループ（任意）','company',c?.company||'','text','maxlength="80"'),v=>{
    const number=phoneNumber(v.number);if(!v.name.trim())return false;
    if(number&&!validPhone(number)){A.toast('電話番号を3〜15桁の数字で入力してください');return false;}
    if(v.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.email)){A.toast('メールアドレスを確認してください');return false;}
    const entry={...c,...v,name:v.name.trim(),number,id:c?.id||A.id()};if(!saveContacts(c?addressBook.map(x=>x.id===id?entry:x):[...addressBook,entry]))return false;A.actions.contactsHome();
  });}
  A.actions.contactNew=()=>contactEditor();A.actions.contactEdit=()=>contactEditor(selectedContact);
  A.actions.contactDetail=el=>{
    const c=addressBook.find(c=>c.id===el.dataset.id);if(!c)return;selectedContact=c.id;
    A.view(A.nav('連絡先',`<button data-action="contactEdit">編集</button>`,'contactsHome','戻る')+`<div class="app-content work-app"><div class="contact-hero"><span class="avatar">${esc(Array.from(c.name)[0])}</span><h1>${esc(c.name)}</h1><p>${esc(c.company||'あなたの連絡先')}</p>${UI.button('contactFavorite',c.favorite?'★ よく使う項目':'☆ よく使う項目に追加')}</div><div class="contact-methods">${c.number?`<article class="insight-card"><small>電話番号</small><h3>${esc(c.number)}</h3><div class="work-toolbar">${UI.button('contactUse','電話で開く','data-value="phone"')}${UI.button('contactUse','SMSを書く','data-value="messages"')}</div></article>`:''}${c.email?`<article class="insight-card"><small>メール</small><h3>${esc(c.email)}</h3>${UI.button('contactUse','メールを書く','data-value="mail"')}</article>`:''}</div><div class="work-toolbar">${UI.button('contactDelete','連絡先を削除')}</div><p class="work-note">ここで発信・送信はされません。移動先で確認してください。</p></div>`);
  };
  A.actions.contactFavorite=()=>{if(saveContacts(addressBook.map(c=>c.id===selectedContact?{...c,favorite:!c.favorite}:c)))A.actions.contactDetail({dataset:{id:selectedContact}});};
  A.actions.contactDelete=()=>A.confirm('連絡先を削除','このブラウザの連絡先から削除します。',()=>{if(saveContacts(addressBook.filter(c=>c.id!==selectedContact)))A.actions.contactsHome();});
  const fillRecipient=(c,target)=>{const field=target==='mail'?$('#external-mail [name=to]'):target==='messages'?$('#external-message [name=number]'):$('#live-phone-number');if(field){field.value=target==='mail'?c.email:c.number;field.dispatchEvent(new Event('input',{bubbles:true}));field.focus();}};
  A.actions.contactUse=el=>{const c=addressBook.find(c=>c.id===selectedContact);if(!c)return;A.open(el.dataset.value);fillRecipient(c,el.dataset.value);};
  A.actions.contactPicker=()=>{const target=A.current,items=addressBook.filter(c=>target==='mail'?c.email:c.number);A.overlay(`${A.overlayTitle('宛先を選ぶ')}<div class="connected-overlay">${items.map(c=>contactRow(c,'contactPick')).join('')||'<p class="connected-caption">電話アプリの「連絡先を管理」から、宛先を登録してください。</p>'}<p class="connected-caption">登録した宛先を入力欄に入れます。自動送信はしません。</p></div>`);};
  A.actions.contactPick=el=>{const c=addressBook.find(c=>c.id===el.dataset.id);if(!c)return;A.closeOverlay();fillRecipient(c,A.current);};
  ...
  const capabilities = {
    ...
    games:['端末内で実動作','外部接続を必要としない3ゲーム。スコアは端末内保存。'],
  ...
  N.calendarEvent = event => { const start = new Date(`${event.date}T${event.time || '00:00'}:00`); if (!Number.isFinite(start.getTime())) return null; const explicitEnd=event.endTime?new Date(`${event.date}T${event.endTime}:00`):null; const end=explicitEnd&&explicitEnd>start?explicitEnd:new Date(start.getTime()+3600000); return {start,end,google:'https://calendar.google.com/calendar/render?' + new URLSearchParams({action:'TEMPLATE',text:event.title || '予定',dates:`${dateICS(start)}/${dateICS(end)}`,location:event.place || '',details:event.description||'auraから作成。保存前に日時を確認してください。'})}; };
  A.actions.calendarExchange = () => A.overlay(`${A.overlayTitle('予定を外部で使う')}<div class="connected-overlay"><p class="connected-caption">開始時刻は端末のタイムゾーン（${esc(Intl.DateTimeFormat().resolvedOptions().timeZone)}）、終了は指定時刻（未指定なら1時間後）。移動先で確認・保存してください。変更の同期はありません。</p>${button('calendarICS','全予定をICSで書き出す')}${(A.allEvents?.() || []).map(event => { const info = N.calendarEvent(event); return info ? `<div class="connection-divider">${esc(event.date)} ${esc(event.time)} · ${esc(event.title)}</div>${N.link(info.google,'Google カレンダーで作成')}` : ''; }).join('') || '<p>予定がありません。</p>'}</div>`);
  A.actions.calendarICS = () => { const lines = (A.allEvents?.() || []).flatMap(e => { const info = N.calendarEvent(e); return info ? ['BEGIN:VEVENT',`UID:${icsEscape(e.id)}@aura.local`,`DTSTAMP:${dateICS(new Date())}`,`DTSTART:${dateICS(info.start)}`,`DTEND:${dateICS(info.end)}`,`SUMMARY:${icsEscape(e.title)}`,`LOCATION:${icsEscape(e.place)}`,`DESCRIPTION:${icsEscape(e.description)}`,'END:VEVENT'] : []; }); if (!lines.length) return A.toast('書き出せる予定がありません'); icsDownload(lines,'aura-calendar.ics'); };
  A.actions.exportReminders = () => icsDownload(A.searchableReminders().flatMap(r => ['BEGIN:VTODO',`UID:${icsEscape(r.id)}@aura.local`,`DTSTAMP:${dateICS(new Date())}`,`SUMMARY:${icsEscape(r.text)}`,...(r.due?[`DUE;VALUE=DATE:${r.due.replace(/-/g,'')}`]:[]),`PRIORITY:${r.priority==='high'?1:0}`,`DESCRIPTION:${icsEscape(r.detail)}`,`STATUS:${r.done ? 'COMPLETED' : 'NEEDS-ACTION'}`,'END:VTODO']),'aura-reminders.ics');
