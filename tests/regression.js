...
frame.addEventListener('load',async()=>{
 ...
 await test('Voice recorder permission is opt-in',()=>{A.open('recorder');assert(d.querySelector('#record-button[data-action="recordToggle"]'),'Record control missing');assert(!d.querySelector('.recording'),'Unexpected recording started');});
 ...
 // Health stores timestamped records (v2); totals are derived per day and saves are async.
 await test('Health hydration update',async()=>{A.open('health');const before=A.healthData().water||0;click('[data-action="healthWater"]');await delay(150);assert(A.healthData().water===before+200,'Water update');assert(A.load('health',{}).records.some(r=>r.kind==='water'),'Water record not stored');});
 ...
 await test('Health rings follow records and failed saves retain values',async()=>{
 ...
 await test('Zero health activity draws empty rings without fake progress',()=>{
  ...
  A.save('health',{version:2,records:[],session:null,cup:200});A.open('health');
  for(const ring of [...d.querySelectorAll('.health-progress-ring')]){assert(ring.getAttribute('stroke-dasharray')==='0 100','Nonzero empty ring');assert(ring.getAttribute('stroke-linecap')==='butt','Empty ring has a visible cap');}
 });
